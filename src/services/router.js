...
class Router {
  ...
  _findParamValue(pathValue, params = {}) {
    const match = pathValue.match(PARAM_REGEX)
    let result = ''
    if (Array.isArray(match) && match.length > 0 && params.hasOwnProperty(match[1])) {
  ...
  updateRouteParams(params = {}) {
    ...
    window.history.pushState({}, document.title, `${routePath}${searchParams.length ? `?${searchParams}` : ''}`)
    return this
  }

  removeURLSearchParams() {
    window.history.replaceState({}, document.title, this.getCurrentPath())
  }
